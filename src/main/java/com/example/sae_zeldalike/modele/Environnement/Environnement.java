...
public class Environnement {
    ...
    /*public boolean estDansLimiteTerrain(int x, int y){
        for(int i = 0 ; i < this.map.getCoordonnéesToutesTuiles().size() ; i++){
            if(x > this.map.getCoordonnéesToutesTuiles().get(i)[1] ||
            x < this.map.getCoordonnéesToutesTuiles().get(i)[1] ||
            y > this.map.getCoordonnéesToutesTuiles().get(i)[0] ||
            y < this.map.getCoordonnéesToutesTuiles().get(i)[0]){
                return true;
            }
        }
    ...
    public boolean estDevantObstacle(int x, int y) {
        for (int i = 0; i < this.map.getCoordonnéesTuilesNonTraversables().size(); i++) {
            if (x < this.map.getCoordonnéesTuilesNonTraversables().get(i)[1] + 25 &&
                    x + 25 > this.map.getCoordonnéesTuilesNonTraversables().get(i)[1] &&
